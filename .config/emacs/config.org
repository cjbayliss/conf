...
** Eshell
   ...
   #+begin_src emacs-lisp
     (defun eshell/mpv (&rest args)
       "mpv's output is problematic in eshell, so run as
     async-shell-command. ARGS passed to mpv."
       (if args
           (async-shell-command (concat "mpv " (string-join args " ")))
         (shell-command-to-string "mpv --help")))
   #+end_src
